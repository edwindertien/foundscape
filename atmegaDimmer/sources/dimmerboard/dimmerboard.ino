...
//*****************************************************************************
#include <EEPROM.h>
// as low baudrate in previous systems 57600 has been used, 1000000 is necessary for (for example) Dynamixel Servos
#define BAUDRATE_LOW 115200
#define BAUDRATE_HIGH 1000000
//#define ORDER_RYG (1)  // soldered LEDs might end up in different order
#define ORDER_YGR (1)
//#define ORDER_GYR (1)
//#define LED_ADDRESS_COUNT (1)

int basenote; // we store this in EEPROM so it can be changed by special RS485 message
int eeprom_address = 1;   // just a spot. Pick a new one after 100000 cycles :)
#define BASE_CHANNEL 0 // midi channel
// Uncomment the following to change the behaviour regarding 'broadcast' channels (i.e. everything not in the channel slot the system is assigned to
//#define BROADCAST (1)
...
void setup() {
  ...
  boardAddress = 4 * BASE_CHANNEL + digitalRead(4) + 2 * digitalRead(5);
  mode = digitalRead(6);
  if (digitalRead(7)) Serial.begin(BAUDRATE_LOW); else Serial.begin(BAUDRATE_HIGH);
  //
  if (mode)LedControlInit(); else ControlInit();

  EEPROM.get(eeprom_address, basenote);
  //if (basenote < 0 || basenote > 120) basenote = BASE_NOTE;
#ifdef LED_ADDRESS_COUNT
  blinkRed(basenote);
#endif
...
void ProcessData(unsigned char DataLengthBuffer, unsigned char* Data)
{
  ...
  if (Data[0] == 0x03) {
    ...
    if (Data[1] >= basenote && Data[1] < basenote + N_CHANNELS) {
      if (mode) ProcessDimmerCommand(constrain(Data[1] - basenote, 0, 127), Data[2], 1);
      else ProcessNewCommand(constrain(Data[1] - basenote, 0, 127), Data[2]);
    }
  }
  else if (Data[0] == 0x05) {
    if (DataLengthBuffer != 0x0B) return; // this is a group message, 1 + 8 + 2 bytes
    if (Data[1] == basenote) { // then it's for this board: 0, 8, 16, 24, 32, etc...
      for (int i = 0; i < N_CHANNELS; i++) {
        if (mode) ProcessDimmerCommand(i - basenote, Data[i + 2], 1);
        else ProcessNewCommand(i - basenote, Data[i + 2]);
      }
    }
  }
  else if (Data[0] == 0x06) {
    if (DataLengthBuffer != 0x04) return;
    if (Data[1] == 0) {
      if (Data[2] >= 0 && Data[2] < 120) {
        digitalWrite(LED_RED, LOW);
        basenote = Data[2];
        EEPROM.put(eeprom_address, basenote);
